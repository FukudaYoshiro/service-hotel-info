...
<?php
  ...
  if (tep_db_num_rows($admins_check_query) > 0) {
    ...
    $contents[] = array('text' => TEXT_USERNAME . '<br />' . tep_draw_input_field('username','','class="long_text"'));
    $contents[] = array('text' => '<br />' . TEXT_PASSWORD . '<br />' . tep_draw_password_field('password','',false,'class="long_text"'));
    $contents[] = array('align' => 'left', 'text' => '<br />' . tep_draw_button(BUTTON_LOGIN, 'key'));
  } else {
    $heading[] = array('text' => '<strong>' . HEADING_TITLE . '</strong>');

    $contents = array('form' => tep_draw_form('login', FILENAME_LOGIN, 'action=create'));
    $contents[] = array('text' => TEXT_CREATE_FIRST_ADMINISTRATOR);
    $contents[] = array('text' => '<br />' . TEXT_USERNAME . '<br />' . tep_draw_input_field('username','','class="long_text"'));
    $contents[] = array('text' => '<br />' . TEXT_PASSWORD . '<br />' . tep_draw_password_field('password','',false,'class="long_text"'));
